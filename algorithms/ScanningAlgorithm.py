...
class ScanningAlgorithm(object):
    ...
    def processFrame(self, frame, show=False):
        ...
        # point at bottom middle of the screen
        frame = cv2.circle(frame, (self.WIDTH // 2, self.lower_y_bound), 5, (0, 255, 0), -1)

        # line between the two points above
        frame = cv2.line(frame, (self.WIDTH // 2, self.lower_y_bound), (vanishing_point[0], self.upper_y_bound), (0, 255, 0), 2)

        # finding the angle between the center of the frame and the line drawn to the vanishing point
        up_vec = [0, 1]
        dir_vec = [vanishing_point[0] - self.WIDTH // 2, self.upper_y_bound]
        angle = np.arccos(np.dot(up_vec, dir_vec) / (np.linalg.norm(up_vec) * np.linalg.norm(dir_vec))) * 180 / np.pi
        if vanishing_point[0] > self.WIDTH // 2:
            angle = -angle
        # print(angle)
